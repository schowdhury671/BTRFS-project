...
using namespace std;
#define max_entry 5 // let us assume that a node can have atmost 5 entries
...
ll node_ct; // stores number of nodes currently present in tree
...
struct btreeNode {
    ...
    //char timestamp[100];  // denotes the last modified time of the node
...
/* creating new node */
struct btreeNode * createNode(int val, struct btreeNode *child, struct btreeNode *root) {
    ...
    //newNode->st = "F";
    // time_t now = time(0); // stores the time instance in which the node is created 
    // strcpy(newNode->timestamp, ctime(&now));
    newNode->node_id = node_ct++;  // stores the node id 
    return newNode;
}
...
struct btreeNode * insertion(int val, struct btreeNode *root) {
    int flag, i;
    struct btreeNode *child;
    //cout << "insertion here" << val << endl;
...
void deletion(int val, struct btreeNode *myNode, struct btreeNode *root) {
